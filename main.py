...
def go_north(points):
    print_with_delay("You have reached a huge tree. You need to choose another direction.")
    points -= 1  # Decrease points by one
    print_with_delay(f"Your current points: {points}")
    return points

def go_east(points):
    print_with_delay("You have arrived at a stream. Looks like you are on the right path!")
...
def go_south(points):
    print_with_delay("You encountered an angry gorilla. You need to escape to another direction.")
    points -= 1  # Decrease points by one
    print_with_delay(f"Your current points: {points}")
    return points
...
def explore_cave(points):
    ...
    choice = input("Type 'yes' if you want to enter, 'no' if you don't: ").strip().lower()
    if choice == 'yes':
        print_with_delay("You entered the cave and encountered a strange monster!")
        points -= 2  # Decrease points by two
        print_with_delay(f"Your current points: {points}")
    else:
        print_with_delay("You decided not to explore the cave and continued your journey.")
        print_with_delay(f"Your current points: {points}")
    return points

def find_treasure(points):
    print_with_delay("You found a treasure buried in the soil! Just as you expected, you gained 5 additional points.")
...
def forest_escape_game():
    print_with_delay("You wake up and find yourself in a scary forest full of tall trees and you hear scary sounds around you.")

    print_with_delay("There is a deserted road and a valley There is also a large,dark cave, and there is a treasure map lying on the ground")
    ...
    if points >= 3:
        print_with_delay("\nCongratulations! You won the game! You managed to escape from the forest with points:", points)
    else:
        print_with_delay("\nUnfortunately, you failed to escape from the forest. Final points:", points)

def play_forest_escape_game():
    while True:
        forest_escape_game()
        print_with_delay("\nDo you want to play again?")
        play_again = input("Type 'yes' if you want to play again, or 'no' if you don't: ").strip().lower()
        if play_again != 'yes':
